...
module kdtree2_priority_queue_module
  use iso_fortran_env, only : kdkind => real32
  implicit none
  !
...
module kdtree2_module
  ...
  ! don't make sr a global variable (not thread safe!)
...
contains
  ...
  function kdtree2_create(input_data,dim,sort,rearrange) result (mr)
    ...
    ! ..
    allocate (mr)
    allocate (mr%the_data, source=input_data)
...
!   endif
  ...
  subroutine validate_query_storage(sr,n)
    ...
    !
    integer, intent(in) :: n
    type(tree_search_record), intent(in), target :: sr
